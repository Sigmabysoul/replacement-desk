...
/** Collects Logistics' shipping label and product proof photos in one handoff. */
    ...
    <form action={action} className="grid gap-4">
      ...
      <div className="rounded-xl bg-indigo-50 p-4">
        <p className="font-bold text-indigo-950">
          {requiresLabel ? "Add the label and product photos" : "Resubmit product photos"}
        </p>
        <p className="mt-1 text-sm text-indigo-800">
          Logistics supplies the evidence. Printing and Packing will handle the next steps.
        </p>
      </div>
      ...
      <label className="relative flex min-h-28 cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-indigo-300 bg-indigo-50/70 p-4 text-center transition hover:border-indigo-500 hover:bg-indigo-50">
        ...
        <input
          ...
          className="absolute inset-0 cursor-pointer opacity-0"
          aria-label="Upload product proof photos"
        />
      </label>
      ...
      <SubmitButton disabled={photos.length === 0 || (requiresLabel && !labelName)} pendingText="Submitting files…">
        <Send className="size-5" />
        {requiresLabel ? "SUBMIT TO PRINTING" : "RESUBMIT PRODUCT PHOTOS"}
      </SubmitButton>
    </form>
  );
}
