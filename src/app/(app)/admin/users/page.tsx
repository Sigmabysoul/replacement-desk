...
export default async function UsersPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  return <div className="grid gap-6"><div><p className="text-sm font-bold text-indigo-700">ADMIN</p><h1 className="mt-1 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">Users</h1><p className="mt-1 text-sm text-slate-600">Invite workers and control their role and access.</p></div><Notice>{notice.error ?? (error ? "Could not load users." : undefined)}</Notice><Notice tone="success">{notice.success}</Notice>
    ...
    <section><div className="mb-3 flex items-center gap-2"><Users className="size-5 text-slate-500" /><h2 className="font-black text-slate-950">Team</h2></div><div className="grid gap-3">{users.map((user) => <Card key={user.id} className="p-4"><form action={updateUserAction} className="grid items-end gap-3 sm:grid-cols-[1fr_180px_160px]"><input type="hidden" name="id" value={user.id} /><Field label="Full name" hint={user.id}><Input name="full_name" required maxLength={120} defaultValue={user.full_name} /></Field><Field label="Role"><Select name="role" defaultValue={user.role} aria-label={`Role for ${user.full_name}`}>{ROLES.map((role) => <option key={role}>{role}</option>)}</Select></Field><div className="grid grid-cols-2 gap-2"><Select name="active" defaultValue={String(user.active)} aria-label={`Access for ${user.full_name}`}><option value="true">Active</option><option value="false">Inactive</option></Select><button className="min-h-12 rounded-xl bg-slate-900 px-3 text-sm font-bold text-white">SAVE</button></div></form></Card>)}</div></section>
  </div>;
}
