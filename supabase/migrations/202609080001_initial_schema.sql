...
create type public.app_role as enum ('ESHA', 'PRINTING', 'PACKING', 'ADMIN');
...
create policy "esha and admin can create replacements" on public.replacements for insert to authenticated with check (public.current_active_role() in ('ESHA', 'ADMIN') and created_by = auth.uid() and status = 'NEW');
...
create policy "authorized users can add request files" on public.attachments for insert to authenticated with check (
  ...
  ((public.current_active_role() in ('ESHA', 'ADMIN')) and attachment_type in ('CUSTOMER_PHOTO', 'LABEL', 'OTHER'))
);
...
begin
  ...
  elsif p_target_status in ('QC_APPROVED', 'QC_REJECTED') and not (current_row.status = 'QC_PENDING' and actor_role in ('ESHA', 'ADMIN')) then raise exception 'Only Esha can review pending QC';
  ...
  elsif p_target_status = 'SHIPPED' and not (current_row.status in ('PACKED', 'NEEDS_TOKEN') and actor_role in ('ESHA', 'ADMIN')) then raise exception 'Only packed replacements can be shipped by Esha';
  elsif p_target_status = 'NEEDS_TOKEN' and not (current_row.status = 'PACKED' and actor_role in ('ESHA', 'ADMIN')) then raise exception 'Only packed replacements can need a token';
